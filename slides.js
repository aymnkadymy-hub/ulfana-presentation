...
let touch=null;$('#deck').addEventListener('touchstart',e=>{if(e.target.closest('button,a,input'))return;const t=e.touches[0];touch={x:t.clientX,y:t.clientY};},{passive:true});$('#deck').addEventListener('touchend',e=>{if(!touch)return;const t=e.changedTouches[0],dx=t.clientX-touch.x,dy=t.clientY-touch.y;if(Math.abs(dx)>65&&Math.abs(dx)>Math.abs(dy)*1.6)go(current+(dx>0?1:-1));touch=null;},{passive:true});
